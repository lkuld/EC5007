...
*This tutorial is a very brief introduction to the basics of statistical tests. It is a rapid succession of complex concepts, it is okay and maybe expected to not understand parts of it at first. The aim is to get some intuition how statistical tests work.*
...
One complication is that we typically do not want to test a hypothesis that specifies a mean and a variance at the same time. If we reject the hypothesis we do not know whether it was due to the mean or variance. Therefore, we typically estimate the variance based on our sample instead of including the variance as a parameter in the hypothesis. As this introduces slightly more variation in the estimate, we have to adjust our theoretical sampling distribution slightly (use a different command in R).
